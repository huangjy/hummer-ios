...
JSValueRef _HMJSCallFunc(id target, NSString *jsFunc, NSArray *args, JSContext *context) {
...
void _HMJSCallSetter(id target, NSString *jsProp, JSValueRef value, JSContext *context) {
    if (!jsProp) { return; }
...
JSValueRef _HMJSCallGetter(id target, NSString *jsProp, JSContext *context) {
...
void HMJSObjectFinalize(JSObjectRef object) {
    ...
    if (objectRef) CFRelease(objectRef);
}

JSValueRef HMJSCallStaticFunction (JSContextRef contextRef, JSObjectRef functionRef, JSObjectRef thisObject, size_t argumentCount, const JSValueRef arguments[], __unused JSValueRef *exception) {
    ...
    JSValueRef funcNameRef = JSObjectGetProperty(contextRef, functionRef, JSStringCreateWithUTF8CString("name"), NULL);
    ...
    for (size_t index = 0; index < argumentCount; index++) {
        mutableArguments[index] = [JSValue valueWithJSValueRef:arguments[index] inContext:context];
    }
    
    if (thisObj) {
        return _HMJSCallFunc(thisObj, [funcNameStr toString], mutableArguments.copy, context);
    }
    
    return JSValueMakeUndefined(contextRef);
}

JSValueRef HMJSGetStaticValue(JSContextRef contextRef, JSObjectRef objectRef, JSStringRef propertyName, __unused JSValueRef *exception) {
    JSContext *context = [JSContext contextWithJSGlobalContextRef:JSContextGetGlobalContext(contextRef)];
    ...
    if ([jsProperty isEqualToString:@"methods"]) {
        ...
        if (allMethods) {
            return [JSValue valueWithObject:allMethods inContext:context].JSValueRef;
        }
        ...
        if (allProps) {
            return [JSValue valueWithObject:allProps inContext:context].JSValueRef;
        }
    } else if (thisObj) {
...
bool HMJSSetStaticValue(JSContextRef contextRef, JSObjectRef objectRef, JSStringRef propertyName, JSValueRef value, JSValueRef *exception) {
    JSContext *context = [JSContext contextWithJSGlobalContextRef:JSContextGetGlobalContext(contextRef)];
    NSString *jsProperty = (NSString *)CFBridgingRelease(JSStringCopyCFString(CFAllocatorGetDefault(), propertyName));
